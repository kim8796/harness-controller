...
`v1.8.15` 기준 controller-safe export surface 에 beginner `install/task/run` UX, guided task interview, advisory AI review artifacts, task intake draft/review/queue, external backlog-bound product commit/push gates, managed-latest/xhigh implementation defaults, v1.8+ release note 이력 보존, generated coverage artifact exclusion 이 포함된다. Implementation gate 뒤 backlog 상태는 자동으로 바뀌지 않고 `target backlog transition` 명령에서만 바뀌며, product commit 은 completed backlog 에 대한 `target backlog commit` 명령에서만, product push 는 matching commit receipt 에 대한 `target backlog push` 명령에서만 dry-run first 로 열린다.
