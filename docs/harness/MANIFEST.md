...
`v1.8.19` 기준 controller-safe export surface 에 beginner help home, shorter beginner install prompt, beginner `install/task/list/run/finish` UX, guided task interview, advisory AI review artifacts, task intake draft/review/list/queue, external backlog-bound product commit/push gates, managed-latest/xhigh implementation defaults, v1.8+ release note 이력 보존, generated coverage artifact exclusion 이 포함된다. Bare `./harness` 와 `./harness help` 는 한국어 quick-start home 이고, `./harness --help` 는 advanced argparse reference 다. Implementation gate 뒤 backlog 상태는 자동으로 바뀌지 않고 `finish --apply` 또는 `target backlog transition` 명령에서만 바뀌며, product commit 은 completed backlog 에 대한 `finish --commit --message ... --apply` 또는 `target backlog commit` 명령에서만, product push 는 matching commit receipt 에 대한 `finish --push --apply` 또는 `target backlog push` 명령에서만 dry-run first 로 열린다.
