...
`v1.8.8` 기준 controller-safe export surface 에 external sidecar backlog transition gate 가 포함된다. Implementation gate 뒤 backlog 상태는 자동으로 바뀌지 않고 `target backlog transition` 명령에서만 바뀐다.
