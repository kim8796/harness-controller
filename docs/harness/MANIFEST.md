...
현재 beginner surface 는 `install`, `goal`, `watch`, `status` 중심이다. 여러 target을 보는 운영 projection은 `fleet status`가 맡는다. `do`는 한 작업을 바로 처리하는 helper이고, task/review/queue/run/finish/archive 하위 명령은 내부 strict 계약과 복구/디버깅을 위해 계속 export한다.

`v1.8.32` 기준 controller-safe export surface 에 root-link-safe routed `START_HERE.md`, beginner help home, shorter beginner install prompt, beginner `install/goal/watch` UX, read-only `fleet status`, controller-local global reusable learning memory, bounded watch smoke controls, target-scoped watch status heartbeat, last transaction watch status carry-forward, internal operator-wait records, absorbed Harness autonomy policy, extracted `scripts/harness_watch.py` orchestration, extracted `scripts/harness_task_cli.py` task/do orchestration, controller-local runtime readiness/setup guidance, goal sidecar store/planner/refill, guided task interview, advisory AI review artifacts, task intake draft/review/list/queue/fix-scope, controller `release-check`, secret-safe `telegram setup` wizard, external autopilot run, task branch PR publication receipt, safe task PR auto-merge/local sync receipt, external backlog-bound product commit gates, controller smoke retention cleanup, controller-owned Upstash relay drain adapter, target-aware relay env readiness, managed-latest/xhigh implementation defaults, v1.8+ release note 이력 보존, generated coverage artifact exclusion 이 포함된다. Bare `./harness` 와 `./harness help` 는 한국어 quick-start home 이고, `./harness --help` 는 advanced argparse reference 다. `./harness install /path/to/product` 는 product repo를 등록하고 controller checkout의 runtime/`.venv` readiness만 다루며 product repo에 runtime/state/secrets를 복사하지 않는다. unsupported OS/Homebrew 없음/non-TTY에서는 runtime auto-install 없이 next action만 보고한다. `watch` operator-wait는 새 beginner command가 아니라 사용자가 해결 가능한 blocker를 sidecar/status/outbox로 표현하는 내부 상태다. `fleet status`는 target별 readiness, goal, backlog, watch, operator-wait, publication/merge, compact learning 상태만 읽고 product repo를 변경하지 않는다. `task review` 의 scope 보정은 named config alias 에만 적용되고 canonical backlog parser 는 계속 strict 하다. `./harness watch` 가 continuous goal loop 이고, bare `./harness run` 과 `finish` 는 복구/디버깅용 고급 wrapper 로 유지된다.
...
- 현재 release 는 `v1.8.32` 이며, repo/bundle-local `./harness` root-link-safe routed beginner docs, beginner help home, shorter beginner install prompt, `./harness new|init|export|upgrade|complete-setup|verify --loop-ready|profiles|env check|env register --dry-run|telegram setup|self doctor|self install|self uninstall|status|dashboard|fleet status|run --once` one-command starter CLI, beginner external controller `install|goal|watch|watch --status|do|task|task list|task interview|task review|task review --ai|task fix-scope|run|finish|smoke implementation` UX, bounded `watch --max-cycles 1 --no-telegram-drain` smoke, controller-owned `targets/<id>/watch/latest.json|latest.md` status with last transaction carry-forward, product setup readiness, target version/release state, and internal operator-wait projection, extracted `scripts/harness_watch.py` orchestration, extracted `scripts/harness_task_cli.py` task/do orchestration, `scripts/harness_fleet.py` multi-target status/global learning projection, operator-wait record/reply/redaction primitive, absorbed Harness policy around planning/evidence/approval boundaries, `./harness controller audit-size|cleanup`, optional global wrapper, `./harness controller doctor|release-check|export`, `./harness target add|remove|alias|set|set-default|clear-default|list|verify|status|dashboard|version|release|run --once|run --plan-once|run --execute-backlog-once|run --implement-backlog-once|target backlog transition|target backlog commit|target backlog push|run --execute-once|run --execute-once --commit|run --execute-once --commit --push` external controller preview, canonical external controller `StatePaths` resolver, target-scoped external run lock, target alias/default selector, RootContext-aware external state plumbing smoke, sidecar backlog plan gate, sidecar task intake interview/draft/review/list/queue/fix-scope, captioned attachment metadata, advisory AI review prompt/schema/response artifacts, backlog-bound product diff smoke, backlog-bound local AI implementation lane with managed-latest/xhigh Codex defaults, goal-driven watch wrapper over implementation/transition/commit/task branch PR publication/safe auto-merge gates, beginner finish recovery wrapper, explicit sidecar backlog transition gate, explicit backlog-bound local product commit gate, legacy explicit backlog-bound product push recovery gate, explicit product diff smoke, local smoke commit gate, advanced smoke push gate, target run read-only/no-op smoke report, detached HEAD smoke blocker, canonical starter profile helper, secret-safe env provider checks including target-aware relay keys, portable starter `create` mode, starter-safe bundle export/upgrade, controller-safe bundle export with release-check, v1.8+ release-history preservation, and generated coverage artifact exclusion, Node 24-compatible controller CI workflow + hosted-runner-safe focused tests + generated controller conftest, starter/controller distribution sanitization report, controller-owned Upstash relay store adapter, secret-safe Telegram setup wizard, signed target-aware opt-in Telegram `/harness` owner relay with external sidecar materialization, compact Korean Telegram outbox cues, Operator Decision Packet v2, portable operator dashboard, adaptive lane timeout budgeting, cleanup debt visibility, deterministic state-apply receipt proof, empty-backlog idle handling, and goal-complete closeout proposal/apply routing 을 포함한다. Installer 와 upgrade 는 live repo state 를 복사하거나 갱신하지 않고 starter-safe files 만 다루며, `harness` executable shim, `scripts/harness_cli.py`, `scripts/harness_watch.py`, `scripts/harness_operator_wait.py`, `scripts/harness_task_cli.py`, `scripts/harness_fleet.py`, `scripts/harness_controller.py`, `scripts/harness_goal.py`, `scripts/harness_product_setup_readiness.py`, `scripts/harness_release.py`, `scripts/harness_publication.py`, `scripts/harness_incident.py`, `scripts/harness_profiles.py`, `scripts/harness_task_intake.py`, `scripts/harness_autonomy/relay.py`, `scripts/harness_relay_store.py`, `scripts/harness_telegram_setup.py` 를 starter/export source 에 포함한다. Cleanup 은 product repo 를 건드리지 않고 controller-owned delete-safe smoke/temp sidecar 만 다룬다. Target remove 는 product repo 를 건드리지 않고 controller target sidecar 등록만 `targets/_archived/` 로 이동한다. Telegram inbound command 는 state 를 직접 바꾸지 않고 embedded mode 에서는 `runs/autonomy/inbox`, external target mode 에서는 `targets/<id>/operator-inbox` decision markdown 만 남긴다.
