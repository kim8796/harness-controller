...
현재 beginner surface 는 `install`, `goal`, `watch`, `status` 중심이다. `do`는 한 작업을 바로 처리하는 helper이고, task/review/queue/run/finish/archive 하위 명령은 내부 strict 계약과 복구/디버깅을 위해 계속 export한다.

`v1.8.31` 기준 controller-safe export surface 에 root-link-safe routed `START_HERE.md`, beginner help home, shorter beginner install prompt, beginner `install/goal/watch` UX, bounded watch smoke controls, target-scoped watch status heartbeat, last transaction watch status carry-forward, internal operator-wait records, absorbed Harness autonomy policy, extracted `scripts/harness_watch.py` orchestration, extracted `scripts/harness_task_cli.py` task/do orchestration, controller-local runtime readiness/setup guidance, goal sidecar store/planner/refill, guided task interview, advisory AI review artifacts, task intake draft/review/list/queue/fix-scope, controller `release-check`, secret-safe `telegram setup` wizard, external autopilot run, task branch PR publication receipt, safe task PR auto-merge/local sync receipt, external backlog-bound product commit gates, controller smoke retention cleanup, controller-owned Upstash relay drain adapter, target-aware relay env readiness, managed-latest/xhigh implementation defaults, v1.8+ release note 이력 보존, generated coverage artifact exclusion 이 포함된다. Bare `./harness` 와 `./harness help` 는 한국어 quick-start home 이고, `./harness --help` 는 advanced argparse reference 다. `./harness install /path/to/product` 는 product repo를 등록하고 controller checkout의 runtime/`.venv` readiness만 다루며 product repo에 runtime/state/secrets를 복사하지 않는다. unsupported OS/Homebrew 없음/non-TTY에서는 runtime auto-install 없이 next action만 보고한다. `watch` operator-wait는 새 beginner command가 아니라 사용자가 해결 가능한 blocker를 sidecar/status/outbox로 표현하는 내부 상태다. `task review` 의 scope 보정은 named config alias 에만 적용되고 canonical backlog parser 는 계속 strict 하다. `./harness watch` 가 continuous goal loop 이고, bare `./harness run` 과 `finish` 는 복구/디버깅용 고급 wrapper 로 유지된다.
