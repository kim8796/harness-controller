...
v1.8.7 부터 controller release hotfix evidence 는 controller bundle 의 `requirements.txt` 포함, starter bundle 의 `requirements.txt` 제외, controller sanitization pass, private controller GitHub Actions pass 여부를 함께 남긴다. v1.8.20 부터 controller release evidence 는 `./harness controller release-check` 결과, source checkout warning 허용 여부, controller distribution strict-mode pass, optional focused lint/pytest pass/fail, forbidden tracked path redaction, and subprocess output redaction 을 함께 남긴다. v1.8.21 부터 starter documentation evidence 는 `START_HERE.md` 가 짧은 routing guide 로 남고 detailed operator/task/Telegram/troubleshooting/scaffold content 가 dedicated docs 로 분리됐는지 함께 남긴다.
