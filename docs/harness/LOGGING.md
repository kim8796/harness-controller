...
v1.7.98 부터 external harness controller preview 작업은 `RootContext(controller_root,target_root,state_root,mode)` 계약, controller-side target registry, product repo no-mutation proof, starter sanitization report, controller-only GitHub Actions template exclusion, generic AGENTS/CLAUDE starter adapter 를 run evidence 에 남긴다. v1.7.107 부터 `./harness target run --once` 는 verified clean target 의 read-only/no-op smoke 성공으로 기록할 수 있지만, 실제 product-changing lane 실행 성공처럼 쓰면 안 된다. v1.7.108 부터 alias/default 작업은 selector 값이 canonical `target_id` 로 해석된 증거와 Redis/signature/inbox 에 alias 가 남지 않는 증거를 함께 남긴다. v1.8.0 부터 external state plumbing 작업은 sidecar-only `runs/harness`, `reports/harness-autonomy`, `operator-outbox`, `state/` write proof 와 product repo clean proof 를 함께 남긴다. v1.8.1 부터 `./harness target run <target> --execute-once` 는 명시 opt-in product diff smoke 로만 기록하며, evidence 는 exact dirty status `?? product-smoke-change.txt`, unchanged HEAD, tracked/ignored/existing path fail-closed, sidecar preflight, `git -C <target-root> clean -f -- product-smoke-change.txt` rollback guidance 를 함께 남긴다. v1.8.2 부터 `--execute-once --commit` evidence 는 exact commit diff `A product-smoke-change.txt`, `HEAD^ == before_head`, clean after status, remote ref unchanged, hooks/GPG signing skip proof, no product pollution, target report/outbox rollback caution, and controller bundle README rollback guidance 를 함께 남긴다. v1.8.3 부터 `--execute-once --commit --push` evidence 는 registered branch upstream, remote before/after SHA, exact `git push --no-verify <remote> HEAD:refs/heads/<branch>` command shape, no-force/no-tags proof, product push automation caution, push rejection behavior, no product pollution, and release checksum/CI proof 를 함께 남긴다. v1.8.4 부터 `--plan-once` evidence 는 selected sidecar backlog id/path, canonical `StatePaths.state_root` discovery, symlink/non-regular backlog file fail-closed, and product status clean proof 를 남긴다. v1.8.5 부터 `--execute-backlog-once` evidence 는 selected sidecar backlog id/path/title, hidden RootContext revalidation, canonical selected item match, pre-write failure no-diff/no-rollback report proof, exact dirty status `?? product-smoke-change.txt`, unchanged HEAD, no backlog completion, no commit/push, no Telegram-triggered execution, and `git -C <target-root> clean -f -- product-smoke-change.txt` rollback guidance 를 함께 남긴다. v1.8.6 부터 `--implement-backlog-once` evidence 는 selected sidecar backlog id/path/title, AI implementer runner prompt/response paths, product before/after HEAD/status, changed product paths, unchanged HEAD, no backlog completion, no commit/push, no Telegram-triggered execution, and implementation rollback guidance 를 함께 남긴다. v1.8.13 부터 같은 evidence 는 default Codex path 가 literal `auto` 모델을 넘기지 않았는지, model strategy, and `xhigh` reasoning effort 를 함께 남긴다. v1.8.19 부터 beginner task list evidence 는 packet id별 review/queue 상태, stale review 감지, canonical backlog discovery linkage, sidecar backlog symlink/parser fail-closed, secret-safe JSON summary, 그리고 product repo no-mutation proof 를 함께 남긴다.
v1.7.99 부터 controller 배포 작업은 `./harness controller export <dir>` 산출물, controller sanitization report, controller CI workflow 포함, starter bundle workflow/test 제외, 실제 `targets/` git-ignore 확인, signed `target_id` relay envelope/key isolation 을 run evidence 에 남긴다. Codex 모델 capacity/provider availability 오류는 하네스 debt/cleanup 결함이 아니라 외부 가용성 residual note 로만 기록한다. v1.7.102 부터 controller bundle 은 Node 24-compatible workflow, hosted-runner-safe focused tests, generated controller-safe `tests/conftest.py` 포함 여부도 run evidence 로 남긴다. v1.7.103 부터 controller target 작업은 `StatePaths` JSON 과 fail-closed target run 증거를 함께 남기고, v1.7.104 부터 target-scoped lock 충돌/해제 증거도 남긴다. v1.7.105 부터 target-aware Telegram/Redis routing 작업은 sidecar `operator-inbox` materialization, unknown target dead-letter, invalid target id rejection, controller bundle relay/bridge focused tests 를 evidence 로 남긴다. v1.7.107 부터 target run read-only smoke 작업은 `target-run-latest.md`, product git status before/after, dirty/branch/detached-head blocker 증거를 남긴다.
v1.8.7 부터 controller release hotfix evidence 는 controller bundle 의 `requirements.txt` 포함, starter bundle 의 `requirements.txt` 제외, controller sanitization pass, private controller GitHub Actions pass 여부를 함께 남긴다. v1.8.20 부터 controller release evidence 는 `./harness controller release-check` 결과, source checkout warning 허용 여부, controller distribution strict-mode pass, optional focused lint/pytest pass/fail, forbidden tracked path redaction, and subprocess output redaction 을 함께 남긴다. v1.8.23 부터 starter documentation evidence 는 root `START_HERE.md` 와 `docs/harness/START_HERE.md` 링크가 각각의 위치 기준으로 깨지지 않고, detailed operator/task/Telegram/troubleshooting/scaffold content 가 dedicated docs 로 분리됐는지, task scope normalization/fix-scope 가 documented/exported 되었는지 함께 남긴다.
v1.8.8 부터 external backlog transition evidence 는 `target backlog transition` dry-run/apply 결과, source/target sidecar backlog path, status target, reason, implementation evidence run id, product diff path match, unchanged HEAD proof, and sidecar-only receipt path 를 함께 남긴다. v1.8.9 부터 external backlog commit evidence 는 `target backlog commit` apply 결과, implementation run id, completed backlog id/path, product diff fingerprint, before/after HEAD, commit SHA/diff, no-push state, rollback guidance 를 함께 남긴다. v1.8.12 부터 external backlog push evidence 는 `target backlog push` dry-run/apply 결과, matching commit receipt, remote before/after SHA, exact no-force push command shape, no automatic remote rollback caution, remote drift fail-closed proof 를 함께 남긴다. v1.8.24 부터 beginner autopilot evidence 는 transaction-local implementation run id, sidecar completion, product commit receipt, push gate pass/block, repeated failure incident signature, and no product pollution proof 를 함께 남긴다. v1.8.24 controller cleanup evidence 는 `audit-size`, dry-run candidate set, delete-safe smoke/temp sidecar apply receipt, protected reason, and product repo no-delete proof 를 함께 남긴다. v1.8.25 부터 relay drain evidence 는 controller-owned `scripts/harness_relay_store.py`, `--drain-relay --target-id <target>` smoke, unscoped external fail-closed proof, target-aware env readiness redaction, and no external app RedisStore import proof 를 함께 남긴다. v1.8.11 부터 controller export/release evidence 는 v1.8+ release notes preservation, older embedded release-note exclusion proof, generated coverage artifact exclusion proof 를 함께 남긴다. v1.8.23 부터 beginner task intake evidence 는 bare `./harness task` guided interview, captioned image metadata, deterministic `task review`, intake-only safe config scope normalization, `task fix-scope` dry-run/apply proof, advisory-only `task review --ai`, no model execution, no deterministic `review.json` rewrite by AI, secret/injection/broad-glob fail-closed coverage, and `queue --auto` canonical gate preservation 을 함께 남긴다. v1.8.16 부터 beginner finish evidence 는 bare `./harness finish` dry-run, controller-owned implementation evidence discovery, `finish --apply` sidecar-only completion, `finish --commit/--push` dry-run-first behavior, explicit `--apply` mutation, no Telegram-triggered execution, and exported-controller dry-run smoke 를 함께 남긴다. v1.8.17 부터 beginner help evidence 는 bare `./harness` 와 `./harness help` 가 같은 한국어 시작 화면을 내고 target state 를 읽거나 쓰지 않는지, `./harness --help` / subcommand help 는 argparse reference 로 남는지, exported controller bundle 에서도 같은 동작이 유지되는지 함께 남긴다. v1.8.18 부터 beginner install evidence 는 positional repo form, TTY-only prompt, non-TTY status preservation, and mismatched positional/`--repo` fail-closed behavior 를 함께 남긴다.
