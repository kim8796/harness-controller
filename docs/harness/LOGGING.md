...
v1.7.99 부터 controller 배포 작업은 `./harness controller export <dir>` 산출물, controller sanitization report, controller CI workflow 포함, starter bundle workflow/test 제외, 실제 `targets/` git-ignore 확인, signed `target_id` relay envelope/key isolation 을 run evidence 에 남긴다. Codex 모델 capacity/provider availability 오류는 하네스 debt/cleanup 결함이 아니라 외부 가용성 residual note 로만 기록한다. v1.7.100 부터 controller bundle 은 이 workflow 가 실행할 focused tests 와 generated controller-safe `tests/conftest.py` 포함 여부도 run evidence 로 남긴다.
