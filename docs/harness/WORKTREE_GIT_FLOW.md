...
`v1.8.11` external controller baseline 은 completed sidecar backlog 에 연결된 implementation diff 만 product repo local commit 으로 닫고, controller export 는 v1.8+ release note 이력을 보존하며 generated coverage artifact 를 제외한다. Sidecar backlog state 는 여전히 `target backlog transition` 에서만 바뀌며 product push 는 별도 future gate 다.
