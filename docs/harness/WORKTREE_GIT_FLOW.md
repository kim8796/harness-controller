...
v1.7.98 기준 external controller target 은 repo-managed `.worktrees/` 가 아니다. Controller 의 `targets/<id>/` sidecar 는 harness operational state root 이며 product repo worktree cleanup 대상으로 보면 안 된다. Product repo 에 tracked `HARNESS.md`, `harness`, `scripts/harness*`, `runs/**`, `reports/**`, `backlog/**` 같은 embedded harness marker 가 있으면 `target add/verify` 는 fail-closed 하거나 warning 을 남긴다. 외부 target run 은 RootContext-aware core 가 승격되기 전까지 preflight/dashboard 만 만들고 product branch/worktree 를 만들지 않는다.
v1.7.99 기준 controller repo seed 는 `./harness controller export <dir>` 로 만든 controller-safe bundle 만 사용한다. 이 bundle 은 controller CI workflow 를 포함하지만 `.env*`, `targets/**`, live autonomy/report state 를 포함하면 안 된다. v1.7.101 부터 controller bundle 은 workflow 가 실행할 hosted-runner-safe focused tests 와 generated controller-safe `tests/conftest.py` 도 포함한다. Telegram/Redis relay 는 target-aware external 운영에서 signed `target_id` 와 target-scoped Redis keys 를 써야 하며, product worktree cleanup 과 controller sidecar cleanup 을 섞지 않는다.
