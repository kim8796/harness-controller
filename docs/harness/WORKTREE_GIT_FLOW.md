...
`v1.8.13` external controller baseline 은 completed sidecar backlog 에 연결된 implementation diff 를 product repo local commit 으로 닫은 뒤, 별도 `target backlog push` dry-run/apply gate 에서만 registered upstream 으로 push 할 수 있다. Implementation gate 의 기본 Codex 호출은 literal `auto` 모델을 넘기지 않고 managed latest/default 모델과 `xhigh` reasoning 을 쓴다. Controller export 는 v1.8+ release note 이력을 보존하며 generated coverage artifact 를 제외한다. Sidecar backlog state 는 여전히 `target backlog transition` 에서만 바뀐다.
