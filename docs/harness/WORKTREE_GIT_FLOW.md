...
`v1.8.26` external controller baseline 은 bare `./harness` 와 `./harness help` 로 한국어 beginner start home 을 보여주고, root `START_HERE.md` 는 root-relative `docs/harness/...` 링크만 사용한다. `./harness install /path/to/product --id my-app --default` 와 TTY install prompt 를 지원하며, `./harness task list` 로 task packet review/queue 상태와 target-bound 다음 명령을 read-only 로 확인한다. `./harness telegram setup` 은 hard dry-run, ignored env overlay, non-destructive Vercel env sync, separate deploy, fail-closed webhook verification 을 제공한다. `task review` 는 safe config alias 를 intake 단계에서만 exact 후보 파일명으로 보정하고, `task fix-scope` 는 연결된 queued manual-review backlog 를 auto 로 복구할 수 있다. Raw argparse reference 는 `./harness --help` 로 유지한다. Bare `./harness run` 은 default target 의 autopilot wrapper 로 현재 queued auto backlog 를 처리한 뒤 queue 가 비면 종료하고, implementation, sidecar completion, product commit, product push gate 를 기존 gate 로 순서대로 위임한다. Long-running polling 은 `./harness run --watch` 로만 켠다. `./harness finish` 는 autopilot 이 중간에서 멈춘 경우의 복구/고급 wrapper 다. Push 안내는 자동 remote rollback 이 없다는 점을 함께 표시한다. Implementation gate 의 기본 Codex 호출은 literal `auto` 모델을 넘기지 않고 managed latest/default 모델과 `xhigh` reasoning 을 쓴다. Controller export 는 v1.8+ release note 이력을 보존하며 generated coverage artifact 를 제외하고, 배포 전 `./harness controller release-check` 로 distribution strict-mode 와 optional lint/pytest 를 확인한다. `START_HERE.md` 는 짧은 entrypoint 로 두고 상세 문서는 `OPERATOR_GUIDE`, `TASK_INTAKE`, `TELEGRAM`, `TROUBLESHOOTING`, `STARTER_SCAFFOLD` 로 분리한다. Relay drain 은 controller-owned Upstash adapter 로 수행하고, target-scoped external queue 를 명시하지 않으면 root inbox 로 fallback 하지 않는다.
...
starter 문서의 quick start 예시도 위 기본 명령을 기준으로 유지한다. v1.8.25 이후 external controller beginner task flow 는 bare `./harness` 또는 `./harness help` 에서 먼저 보이고, shorter `./harness install <repo> --id ... --default` 로 시작하며, `./harness task` -> 필요 시 `./harness task list/review/queue` -> `./harness run` 순서다. scope-only manual-review dead end 는 `./harness task fix-scope <packet-id> --apply` 로 복구할 수 있다. list/review/AI review/fix-scope 단계는 controller sidecar artifact 만 쓰고 product worktree 를 변경하지 않는다. `run` 은 기존 gate 를 통해 상태/commit/push 를 바꾸며, `finish` 는 복구/고급 경로다. Relay smoke 는 `python3 scripts/harness_telegram_bridge.py --drain-relay --target-id <target> --json` 로 target-scoped 로만 확인한다. Release manager 는 private controller repo push/tag 전 `./harness controller release-check --run-lint --run-pytest` 를 같은 checkout 에서 돌려 tracked forbidden path, ignored `targets/`, cache artifact, and redacted output 상태를 확인한다.
