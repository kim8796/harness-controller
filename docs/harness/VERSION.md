...
- Compatibility: Codex + Claude primary profile + AI-agnostic canonical contract + routed Start Here docs + beginner install prompt + beginner help home + beginner `install/goal/watch` controller UX + localized document-first goal specs + fleet status projection + global reusable learning memory + bounded watch smoke + target-scoped watch status heartbeat + last transaction watch status carry-forward + internal operator-wait watch behavior + absorbed Harness autonomy policy + extracted watch orchestration module + extracted task/do CLI module + task interview + task packet status visibility + task scope normalization + controller release-check gate + advisory AI review artifacts + one-command starter CLI + optional global wrapper + secret-safe env provider checks + target-aware Telegram/Redis env readiness + `telegram setup --dry-run` readiness entrypoint + controller-owned Upstash relay drain adapter + thin adapters + root-canonical `main` checkout + repo-local recovery state + unattended CLI autonomy loop + adaptive lane timeout budgeting + canonical Telegram `/harness` owner instruction inbox + Operator Decision Packet v2 + cleanup debt visibility + successful-cycle commit/push/persistent-branch backup + external Doctor/launcher failure repair and publication boundary + manager scope contract + builder-owned manifest/evidence materialization + canonical `goal_state` + deterministic `state-apply` receipt proof + workspace-keyed control-plane cache + policy/state proposal visibility surfaces + guard/recovery/export discipline + append-only and restore-proof run evidence gates + harness LOC budget guard + managed-latest/xhigh external implementation gate + backlog-bound product push gate + external controller autopilot run + controller smoke retention cleanup + controller release-history-preserving export + generated coverage artifact exclusion + on-demand export output.

## What Changed In 1.8.32

- Added `./harness fleet status` and `./harness fleet status --json` as read-only multi-target status projections.
- Added `scripts/harness_fleet.py` to aggregate target readiness, active goal, backlog, watch, operator-wait, and latest publication/merge state without touching product repos.
- Added deterministic reusable learning promotion from target memory events into controller-local `targets/_global/memory/reusable-lessons.jsonl` and `reusable-index.json`.
- Kept global learning compact and secret-safe by promoting allowlisted signals only, not raw logs, run evidence, product file contents, or secret-like values.
- Added localized `./harness goal draft` templates and shorter `./harness goal from <goal-spec.md> [images-or-directories...]` import for detailed document/image-backed product goals.
- Added focused fleet tests and included the new module/test in controller export and release-check coverage.

## What Changed In 1.8.31

- Added `scripts/harness_operator_wait.py` and focused tests to controller/starter export source coverage.
- Projected credential-blocked publication and operator-actionable transaction blockers into bounded `watch` operator-wait status plus local `operator-outbox` cues.
- Extended incident classification with operator-wait metadata and hardened redaction/sidecar containment for Telegram tokens, private operator ids, and symlinked state parents.
- Documented operator-wait as internal `watch` behavior for bounded, operator-actionable blockers such as missing credentials, approvals, dirty repos, or transient provider outages.
- Folded the absorbed operating discipline into Harness policy language: plan before edits, diagnose at the first failing boundary, require generated evidence before completion, and keep approvals inside canonical gates.

## What Changed In 1.8.30

- Preserved last published transaction fields in `targets/<target-id>/watch/latest.json` when later idle heartbeats clear the current backlog/run fields.
- Updated `./harness watch --status` and `watch/latest.md` to show a separate last transaction block while still reporting the current idle state.
- Kept the expanded watch status secret-safe and sidecar-only.

## What Changed In 1.8.29

- Extracted beginner `do` and `task` command orchestration from `scripts/harness_cli.py` into `scripts/harness_task_cli.py`.
- Kept `scripts/harness_cli.py` as the public parser/compatibility facade for existing imports and monkeypatch points.
- Added focused `tests/test_harness_task_cli.py` coverage and included the new module/test in export and release-check paths.

## What Changed In 1.8.28

- Extracted beginner watch/run orchestration and watch status helpers from `scripts/harness_cli.py` into `scripts/harness_watch.py`.
- Kept `scripts/harness_cli.py` as the public CLI compatibility facade with the existing monkeypatch/import surface.
- Added `scripts/harness_watch.py` and `tests/test_harness_watch.py` to controller/starter export coverage and release-check paths.

## What Changed In 1.8.27

- Added visible bounded smoke controls for `./harness watch`: `--max-cycles`, `--idle-seconds`, `--no-telegram-drain`, and `--stop-on-idle`.
- Added `./harness watch --status` and controller-owned `targets/<target-id>/watch/latest.json` / `.md` status artifacts.
- Made no-goal/no-backlog idle states explicit so `watch --stop-on-idle` can exit cleanly during real target smoke tests.
- Added secret-safe watch status reporting for goal, backlog, run, commit, publication, heartbeat, and next-action state.
