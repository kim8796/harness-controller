...
- portable starter 로 새 프로젝트를 만든 경우에도 loop 시작 전에는 `START_HERE.md` 의 quick start 와 first-loop readiness checklist 를 먼저 확인한다.
  - `./harness controller export <dir>` 로 private controller repo 를 seed 하는 경우 v1.7.100 기준 controller bundle 은 controller CI workflow, focused tests, generated controller-safe `tests/conftest.py` 를 함께 포함해야 한다. Starter bundle 은 workflow/test files 를 계속 제외한다.
...
External controller preview 는 product repo 에 harness 파일을 심지 않는 배포 경로다. Controller checkout 에서 `./harness controller doctor`, `./harness target add <id> --repo /path/to/product --branch main`, `./harness target verify <id>`, `./harness target dashboard <id>` 로 target 을 등록하고, state 는 controller 의 ignored `targets/<id>/` sidecar 아래에 둔다. Embedded mode 는 기존처럼 `controller_root == target_root == state_root` 이고, external mode 는 `controller_root`, `target_root`, `state_root` 를 분리한다. 현재 `./harness target run <id> --once` 는 RootContext-aware autonomy core 가 완성될 때까지 lane 을 시작하지 않는 fail-closed preflight 다.
