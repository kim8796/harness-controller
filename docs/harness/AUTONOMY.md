...
현재 external controller 기본 UX는 `./harness do "요청"`과 `./harness watch`다. `task review/queue`, `run`, `finish`, `target archive`는 strict 내부 게이트이자 복구/디버깅 명령으로 유지한다.

`v1.8.26` 기준 external controller beginner entrypoint 는 bare `./harness` 와 `./harness help` 로 같은 한국어 시작 화면을 보여주고, root `START_HERE.md` 와 `docs/harness/START_HERE.md` 링크가 각각의 위치에서 유효해야 한다. `./harness install /path/to/product --id my-app --default` 와 TTY install prompt 를 첫 연결 경로로 둔다. `./harness telegram setup`은 hard dry-run과 separate Vercel deploy/webhook gate를 제공한다. `./harness task list` 는 task packet 의 review/queue 상태와 target-bound 다음 명령을 read-only 로 보여준다. `task review` 는 safe config alias 를 intake-only 로 exact 후보 파일명으로 보정하고, `task fix-scope` 는 scope-only manual-review dead end 를 controller sidecar 안에서 복구한다. 전체 argparse 명령 참조는 `./harness --help` 로 분리한다. `START_HERE.md` 는 짧은 routing guide 로 유지하고, 상세 운영/요구사항/Telegram/troubleshooting/scaffold reference 는 dedicated docs 로 연결한다. `target run --implement-backlog-once` 는 기본 Codex 호출에서 literal `auto` 모델을 넘기지 않고 managed latest/default 모델과 `xhigh` reasoning 을 사용한다. Bare `./harness run` 은 default target 의 autopilot wrapper 로, 현재 queued auto backlog 를 처리한 뒤 queue 가 비면 종료하고 성공 transaction 에서 기존 implementation, sidecar transition, product commit, product push gate 를 순서대로 재사용한다. `run --once` 는 한 backlog transaction 만 처리하는 debug/smoke 모드이고, `run --watch` 는 새 backlog 를 계속 감시하는 명시 long-running 모드다. Product push 는 matching applied commit receipt 와 remote base preflight 를 요구하며, preflight 가 맞지 않으면 commit 이후 멈춘다. Controller export 는 v1.8+ release note 이력을 보존하고 generated coverage artifact 를 제외하며, release 전 `./harness controller release-check` 로 source checkout warning, distribution strict-mode, focused lint/pytest, forbidden tracked paths, and secret-safe output projection 을 확인한다. Telegram/Redis relay drain 은 controller-owned Upstash adapter 를 사용하고, external controller 에 registered target 이 있으면 target-scoped drain 만 허용한다.
...
- External controller beginner UX 에서 `./harness install` 은 global wrapper 설치가 아니라 target 등록/검증/default 설정 wrapper 다. `./harness task` 는 draft/review/queue 단계로 canonical backlog markdown 을 만들고, `./harness run` 은 default target 의 autopilot wrapper 로 현재 queue를 drain한 뒤 종료하며 implementation, completion, local commit, push gate 를 기존 target backlog gate 에 위임한다. 새 runner/parser/ledger 를 만들지 않는다.
