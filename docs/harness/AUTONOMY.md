...
- operator 입력은 file channel 이 기준이고 Telegram 은 bridge 다.
  ...
  - Product bot 의 relay accepted 응답은 적용 완료가 아니라 local loop drain 대기 상태다. 초기 relay 범위에서 `/harness status` 는 local live status snapshot 이 아니며 relay 설정/health 의 제한된 상태와 local `python3 scripts/harness_autonomy.py status` / outbox 확인 안내만 제공한다.
...
v1.8.23 기준 external controller beginner task intake 는 product repo 에 harness state 를 쓰지 않고 controller sidecar 에 draft/review/list/queue/fix-scope artifact 만 남긴다. `./harness task` 는 guided interview 를 열고, `./harness task list` 는 packet id 별 review/queue 상태와 다음 명령을 read-only 로 보여주며, `./harness task review <packet-id>` 가 deterministic queue gate 를 만든다. Safe config alias 는 review 단계에서 exact 후보 파일명으로 보정되지만 canonical backlog parser 는 strict rule 을 유지한다. `./harness task fix-scope <packet-id> --apply` 는 같은 packet 에 연결된 queued manual-review backlog 가 새 deterministic review 에서 auto eligible 일 때만 auto 로 다시 렌더링한다. `./harness task review <packet-id> --ai` 는 기존 fresh review 를 읽어 prompt/schema/optional response artifact 만 생성한다. 이 AI review 는 모델을 실행하지 않고 `review.json` 을 바꾸지 않으며 `queue --auto` 판단에 사용되지 않는다.
...
- External controller beginner UX 에서 `./harness install` 은 global wrapper 설치가 아니라 target 등록/검증/default 설정과 controller-local runtime readiness wrapper 다. `.venv` 와 setup receipt 는 controller checkout 전용이며 product repo나 portability artifact가 아니다. 기본 경로는 `install -> goal -> watch` 이고, `watch` 는 active goal planner/refill, task queue, implementation, completion, local commit, task branch push, PR publication receipt 를 반복한다. macOS + Homebrew + TTY 밖에서는 runtime auto-install을 하지 않고 next action을 보고한다. `./harness task` 와 `./harness run` 은 draft/review/queue 또는 one-shot drain이 필요한 고급/복구 표면이다. 새 runner/parser/ledger 를 만들지 않는다.
