...
- External controller beginner UX 에서 `./harness install` 은 global wrapper 설치가 아니라 target 등록/검증/default 설정 wrapper 다. `./harness task` 는 draft/review/queue 단계로 canonical backlog markdown 을 만들고, `./harness run` 은 default target 의 autopilot loop 로 implementation, completion, local commit, push gate 를 기존 target backlog gate 에 위임한다. 새 runner/parser/ledger 를 만들지 않는다.
