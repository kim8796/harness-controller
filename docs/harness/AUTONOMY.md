...
`v1.8.12` 기준 external controller backlog completion 은 명시 `target backlog transition` gate 에서만 수행되고, product local commit 은 completed backlog 에 대한 명시 `target backlog commit` gate 에서만 수행된다. Product push 는 matching applied commit receipt 와 remote base preflight 를 요구하는 명시 `target backlog push` gate 에서만 수행된다. Controller export 는 v1.8+ release note 이력을 보존하고 generated coverage artifact 를 제외한다.
