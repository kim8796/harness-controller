...
`v1.8.26` 기준 controller-safe bundle 은 root-link-safe routed `START_HERE.md`, beginner help home, shorter beginner install prompt, beginner `install/task/list/run/finish` UX, guided task interview, advisory AI review artifacts, task intake drafts/review/list/queue/fix-scope, controller `release-check`, secret-safe `telegram setup` wizard, external autopilot run, external backlog-bound local product commit/push gates, smoke retention cleanup, controller-owned Upstash relay drain adapter, target-aware relay env readiness, managed-latest/xhigh implementation defaults, v1.8+ controller release note 이력 보존, generated coverage artifact exclusion 을 포함한다. Bare `./harness` 와 `./harness help` 는 한국어 quick-start home 을 보여주고, `./harness --help` 는 고급 argparse reference 로 남는다. Bare `./harness run` 은 default target 의 현재 queued auto backlog 를 처리한 뒤 queue 가 비면 종료하고 successful transaction 에서 기존 implementation, sidecar transition, product commit, product push gate 를 순서대로 재사용한다. `./harness run --watch` 는 새 backlog 를 계속 감시하는 명시 long-running 모드다. Product push 는 matching product commit receipt 와 clean upstream preflight 를 요구하며, preflight 가 맞지 않으면 commit 이후 멈춘다. `finish` 는 복구/고급 wrapper 로 유지한다. `task review` 는 safe config alias 만 exact 후보 파일명으로 보정하고, `task fix-scope` 는 이미 queued manual-review 로 들어간 scope-only dead end 를 controller sidecar 안에서만 복구한다. `START_HERE.md` 는 짧은 entrypoint 로 유지하고 daily commands, task intake, Telegram, troubleshooting, starter scaffold reference 는 각각 dedicated guide 로 연결한다.
