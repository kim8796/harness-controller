...
현재 controller bundle 의 기본 운영 UX는 `./harness install ...`, `./harness goal "제품 목표"`, `./harness watch`다. 실전 smoke 는 `./harness watch --max-cycles 1 --no-telegram-drain` 으로 한 transaction만 돌리고, `./harness watch --status` 로 target-scoped 상태를 본다. 여러 target은 `./harness fleet status` 로 read-only 확인한다. `./harness do "요청"`은 단일 작업 helper이고, 기존 세부 명령은 starter/export 호환성과 복구를 위해 포함하지만 초보 경로에서는 숨긴다.
...
`v1.8.32` 기준 controller-safe bundle 은 root-link-safe routed `START_HERE.md`, beginner help home, shorter beginner install prompt, beginner `install/goal/watch` UX, read-only `fleet status`, controller-local global reusable learning memory, bounded watch smoke controls, target-scoped watch status heartbeat, last transaction watch status carry-forward, internal operator-wait records, absorbed Harness autonomy policy, extracted `scripts/harness_watch.py` orchestration, extracted `scripts/harness_task_cli.py` task/do orchestration, controller-local runtime readiness/setup guidance, goal sidecar store, deterministic planner/refill, task branch PR publication receipts, guided task interview, advisory AI review artifacts, task intake drafts/review/list/queue/fix-scope, controller `release-check`, secret-safe `telegram setup` wizard, external autopilot run, external backlog-bound local product commit/legacy push recovery gates, smoke retention cleanup, controller-owned Upstash relay drain adapter, target-aware relay env readiness, managed-latest/xhigh implementation defaults, v1.8+ controller release note 이력 보존, generated coverage artifact exclusion 을 포함한다. Bare `./harness` 와 `./harness help` 는 한국어 quick-start home 을 보여주고, `./harness --help` 는 고급 argparse reference 로 남는다. `./harness install /path/to/product` 는 product repo 를 등록하고 controller checkout 의 tools/`.venv` readiness 만 확인/준비하며 product repo 에 runtime/state/secrets 를 복사하지 않는다. macOS + Homebrew + TTY 밖에서는 runtime auto-install 없이 next action 만 보고한다. `./harness watch` 는 active goal 이 끝날 때까지 Telegram/task inbox drain, planner refill, implementation, sidecar completion, product commit, task branch push, PR create/update receipt 를 반복한다. `./harness fleet status` 는 등록 target의 readiness, goal, backlog, watch, operator-wait, publication/merge, compact learning 현황을 product repo 변경 없이 보여준다. Operator-wait는 `watch` 안의 bounded internal state이며 새 beginner command가 아니다. `./harness watch --status` 는 `targets/<id>/watch/latest.json` / `.md` 를 읽어 phase, goal, current backlog/run, last published transaction, operator-wait, commit, publication, heartbeat, next action 을 보여준다. Bare `./harness run`, `run --once`, `finish`, `target backlog push` 는 고급 복구/디버깅 wrapper 이며 beginner publication 경로의 기본은 task branch PR receipt다. `task review` 는 safe config alias 만 exact 후보 파일명으로 보정하고, `task fix-scope` 는 이미 queued manual-review 로 들어간 scope-only dead end 를 controller sidecar 안에서만 복구한다. `START_HERE.md` 는 짧은 entrypoint 로 유지하고 daily commands, task intake, Telegram, troubleshooting, starter scaffold reference 는 각각 dedicated guide 로 연결한다.

`v1.7.97` 부터는 문서 복사형 starter 외에 repo-local/bundle-local `./harness` one-command CLI 를 제공한다. 긴 Python 명령은 세부 구현 경로로 남기고, 새 프로젝트 운영자는 먼저 `./harness new`, `./harness init`, `./harness complete-setup`, `./harness verify --loop-ready`, `./harness env check`, `./harness export`, `./harness upgrade` 를 쓴다. `v1.7.98` 부터는 product repo 에 harness runtime 파일을 커밋하지 않는 external controller preview 도 제공한다. `v1.8.14` 부터 controller beginner path 는 `./harness install`, `./harness task`, bare `./harness run`, `./harness smoke implementation` 을 제공하며, 이는 기존 target registry / sidecar backlog / implementation gate 에 위임한다. `v1.8.15` 부터 bare `./harness task` 는 guided interview 를 열고, `task review --ai` 는 packet-local prompt/schema/optional response artifact 만 만들며 모델 실행, deterministic review mutation, queue auto gate bypass 를 하지 않는다. `v1.8.16` 부터 bare `./harness finish` 는 최신 implementation evidence 를 읽어 다음 단계를 안내하고, `finish --apply`, `finish --commit --message ... --apply`, `finish --push --apply` 만 기존 dry-run-first gate 에 적용을 위임한다. `v1.8.17` 부터 bare `./harness` 와 `./harness help` 는 beginner home 을 보여주고 `./harness --help` 는 advanced reference 로 유지한다. `v1.8.18` 부터 `./harness install <repo> --id ... --default` shorter form 과 TTY install prompt 를 지원한다. `v1.8.19` 부터 `./harness task list` 는 task packet 상태와 packet-specific next action 을 read-only 로 보여주고 stale review 와 target-bound command 를 명확히 표시한다. `v1.8.23` 부터 `task review` 는 named config alias 를 intake-only 로 exact path 후보로 보정하고 `task fix-scope` 로 scope-only manual-review dead end 를 복구한다. `v1.8.6` 부터 `target run --implement-backlog-once` 는 선택 sidecar backlog 를 AI implementer 에 넘겨 local product diff 만 만들며 backlog 완료, commit, push 는 하지 않는다. `v1.8.13` 부터 이 구현 gate 는 기본 Codex 호출에서 unsupported literal `auto` 모델을 넘기지 않고 managed latest/default 모델과 `xhigh` reasoning 을 쓴다. `v1.8.8` 부터 `target backlog transition` 은 implementation evidence 를 검증해 sidecar backlog 를 completed / blocked / manual-review 로 바꾸는 별도 dry-run-first gate 다. `v1.8.9` 부터 `target backlog commit` 은 completed sidecar backlog 에 연결된 implementation evidence 와 diff fingerprint 를 검증한 뒤 evidence-listed path 만 local product commit 으로 닫는다. `v1.8.12` 부터 `target backlog push` 는 같은 implementation run 에 대한 applied commit receipt 와 remote head preflight 를 검증한 뒤 registered branch remote 를 갱신할 수 있다. `v1.8.11` 부터 controller export 는 v1.8+ release notes 를 보존하고 generated coverage artifact 를 제외해 private controller repo refresh 가 이전 controller release 이력을 지우거나 stale local coverage file 을 요구하지 않게 한다. 위 commit/push smoke 와 backlog push gate 는 deployment 가 아니고 자동 remote rollback 을 하지 않는다.
...
- `scripts/harness_watch.py` 는 beginner `watch`/`run` orchestration 과 watch status artifact helpers 의 canonical owner 다. CLI 는 parser binding 과 compatibility wrapper 만 유지한다.
...
- `scripts/harness_fleet.py` 는 multi-target read-only status projection 과 deterministic compact global learning promotion 의 canonical owner 다. Product repo에는 상태나 학습 파일을 쓰지 않는다.
...
- `scripts/harness_telegram_setup.py` 는 `./harness telegram setup` wizard 를 맡는다. `--dry-run` 은 모든 apply/deploy/webhook side effect 를 막고, mutation 은 explicit apply flags 에서만 다룬다.
...
- `scripts/harness_relay_store.py` 는 controller-owned Upstash relay drain adapter 이며, 외부 app 내부 RedisStore 를 import 하지 않는다.
...
│   ├── harness_cli.py
│   ├── harness_watch.py
│   ├── harness_operator_wait.py
│   ├── harness_task_cli.py
│   ├── harness_task_intake.py
│   ├── harness_autonomy.py
