...
| product repo 에 harness 파일을 커밋하지 않는다 | `./harness target add <id> --repo /path/to/repo` | external controller preview 다. sidecar 는 controller 의 ignored `targets/<id>/` 아래에 있고, 실제 product-changing lane 실행은 RootContext-aware core 승격 전까지 fail-closed 한다. |
