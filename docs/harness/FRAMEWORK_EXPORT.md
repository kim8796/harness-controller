...
`v1.8.23` 기준 controller-safe bundle 은 root-link-safe routed `START_HERE.md`, beginner help home, shorter beginner install prompt, beginner `install/task/list/run/finish` UX, guided task interview, advisory AI review artifacts, task intake drafts/review/list/queue/fix-scope, controller `release-check`, external backlog-bound local product commit/push gates, managed-latest/xhigh implementation defaults, v1.8+ controller release note 이력 보존, generated coverage artifact exclusion 을 포함한다. Bare `./harness` 와 `./harness help` 는 한국어 quick-start home 을 보여주고, `./harness --help` 는 고급 argparse reference 로 남는다. Implementation run 뒤 backlog 상태는 자동으로 바뀌지 않고 `finish --apply` 또는 `target backlog transition ... --apply` 에서만 바뀌며, product commit 은 completed backlog 에 대해 `finish --commit --message ... --apply` 또는 `target backlog commit ... --apply` 로만 수행한다. Product push 는 matching product commit receipt 와 clean upstream preflight 를 요구하는 `finish --push --apply` 또는 `target backlog push ... --apply` 에서만 수행한다. `task review` 는 safe config alias 만 exact 후보 파일명으로 보정하고, `task fix-scope` 는 이미 queued manual-review 로 들어간 scope-only dead end 를 controller sidecar 안에서만 복구한다. `START_HERE.md` 는 짧은 entrypoint 로 유지하고 daily commands, task intake, Telegram, troubleshooting, starter scaffold reference 는 각각 dedicated guide 로 연결한다.
