...
새 프로젝트 이식의 기본 진입점은 repo-local 또는 starter-bundle-local `./harness` CLI 다. `./harness` 와 `./harness help` 는 한국어 beginner home 을 보여주고, `./harness --help` 는 고급 argparse 명령 참조다. `./harness new <target>` 은 새 git repo 를 만들고 starter scaffold, Telegram/Redis relay-ready env placeholder, bootstrap interview, recovery sync 까지 수행하지만 autonomy loop 는 시작하지 않는다. `./harness init <repo>` 는 clean existing git root 에 설치하며 충돌은 fail-closed 한다. `./harness complete-setup --apply` 는 기존 bootstrap wizard render/approve 를 감싸 제품 문서와 첫 backlog 를 적용한다. `./harness export <dir>` 는 같은 starter pack 을 다른 곳으로 옮길 bundle 을 만든다. Product repo 에 harness runtime/state 를 커밋하지 않는 운영은 controller 에서 `./harness install <repo> --id <id> --default`, `./harness task`, `./harness task list`, `./harness task review <packet-id>`, 선택적 `./harness task review <packet-id> --ai`, `./harness task queue <packet-id> --auto`, 필요 시 `./harness task fix-scope <packet-id> --apply`, `./harness run`, `./harness finish` 를 우선 사용한다. 이 beginner path 는 내부적으로 `target add`, `target set-default`, sidecar backlog interview/draft/review/list/queue/fix-scope, `target run @default --implement-backlog-once`, target backlog transition/commit/push gate 로 위임한다. v1.8.15 부터 bare `./harness task` 는 guided interview 를 열고, `task review --ai` 는 기존 fresh deterministic review 를 읽어 packet-local prompt/schema/optional response artifact 만 만들며 모델 프로세스를 실행하거나 deterministic `review.json` 을 바꾸지 않는다. v1.8.19 부터 `task list` 는 packet id, review 상태, stale review, queued backlog, target-bound next command 를 read-only 로 보여주며 raw request/caption/hash/secret 값을 JSON 에 넣지 않는다. v1.8.23 부터 `task review` 는 safe config alias 를 exact 후보 파일명으로 보정하고 `task fix-scope` 는 scope-only manual-review queue 를 auto 로 복구할 수 있다. 고급 검토는 `./harness target run <id|@alias|@default> --plan-once` 로 하고, 이 명령은 `targets/<id>/backlog` 만 읽고 product repo 를 바꾸지 않는다. v1.8.5 부터 `./harness target run <id|@alias|@default> --execute-backlog-once` 는 같은 sidecar backlog 후보에 묶인 local `product-smoke-change.txt` diff 만 만들며 AI 구현 lane, backlog 완료 처리, commit, push 는 하지 않는다. v1.8.6 부터 `./harness target run <id|@alias|@default> --implement-backlog-once` 는 같은 sidecar backlog 후보를 AI implementer 에 넘겨 local product diff 만 만들며 backlog 완료, commit, push 는 하지 않는다. v1.8.13 부터 이 구현 gate 의 기본 Codex 호출은 literal `auto` 모델을 넘기지 않고 Codex managed latest/default 모델과 `xhigh` reasoning 을 쓴다. 다른 모델이 필요하면 고급 명령에서 `--runner-model <model-id>` 를 명시한다. v1.8.14 부터 bare `./harness run` 이 이 기본 구현 gate 에 위임하고, v1.8.26 부터는 현재 queued auto backlog 를 처리한 뒤 queue 가 비면 종료한다. 새 작업을 계속 감시해야 할 때만 `./harness run --watch` 를 쓴다. v1.8.16 부터 bare `./harness finish` 는 최신 implementation evidence 를 읽어 다음 단계를 안내하고, `finish --apply` 는 sidecar backlog 완료만 적용한다. `finish --commit --message "<msg>"` 와 `finish --push` 는 dry-run 이며, 실제 local commit / remote push 는 각각 `--apply` 를 추가했을 때만 기존 gate 로 위임한다. v1.8.8 부터 sidecar backlog 상태 변경은 `./harness target backlog transition <id|@alias|@default> --status completed|blocked|manual-review` 로 dry-run 한 뒤 `--apply` 에서만 수행한다. v1.8.9 부터 local product commit 은 `./harness target backlog commit <id|@alias|@default> --run <implementation-run> --message "<msg>"` 에서 dry-run 후 `--apply` 로만 수행하고, completed sidecar backlog / matching `Completed-Run` / product diff fingerprint 를 요구한다. v1.8.12 부터 product push 는 `./harness target backlog push <id|@alias|@default> --run <implementation-run>` dry-run 후 `--apply` 에서만 수행하며, matching commit receipt / clean product repo / remote head preflight 를 요구한다. 보고서는 `targets/<target_id>/reports/target-run-latest.md` 이고 smoke rollback 은 `git -C <target_repo> clean -f -- product-smoke-change.txt`, implementation rollback 은 report 의 changed path 별 guidance 를 따른다. `./harness verify --loop-ready` 는 필수 파일, git clean 상태, `.env` 추적 여부, bootstrap 문서/backlog, Telegram/relay readiness 를 secret 값 없이 점검한다. `./harness env check --provider vercel|upstash` 는 외부 env 준비를 present/missing/weak 상태로만 보여준다. `./harness self install` 은 선택형 global wrapper 를 만들지만 canonical entrypoint 는 계속 local `./harness` 다.

External controller release 는 source repo pre-push guard 와 분리한다. `v1.8.20` 부터 private controller repo 에서는 tag/release 전에 `./harness controller release-check --run-lint --run-pytest` 를 실행해 controller export source, `targets/` git-ignore, tracked forbidden state/secrets, focused lint/test 를 확인한다. `v1.8.23` 부터 root `START_HERE.md` 와 `docs/harness/START_HERE.md` 는 각각의 위치 기준으로 링크가 유효해야 하며, 이식 안내는 짧은 `START_HERE.md` 에서 시작하고 operator commands / task intake / Telegram / troubleshooting / starter scaffold reference 로 분리된 문서에 연결한다. `v1.8.24` 부터 bare `./harness run` 은 external controller autopilot wrapper 로 기존 implementation/transition/commit/push gate 를 재사용하며, `v1.8.26` 부터 empty queue 에서 종료하고 long-running polling 은 `--watch` 로만 켠다. `./harness controller audit-size|cleanup` 은 controller-owned smoke/temp sidecar 만 정리한다. 이 release-check 명령은 read-only 이며 GitHub release, product repo, target sidecar, env 값을 바꾸지 않는다.
