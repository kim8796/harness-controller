...
External controller preview 는 product repo 에 harness runtime 파일을 커밋하지 않기 위한 새 경계다. `./harness controller doctor` 는 controller checkout 을 점검하고, `./harness target add <id> --repo <path>` 는 product git repo 를 controller 의 ignored `targets/<id>/` sidecar 에 등록한다. 이 preview 는 registry/projection/preflight 와 v1.8.0 RootContext state plumbing smoke 를 제공하며, 기존 backlog/GOALS/run parser 를 복제하지 않는다. 실제 product-changing autonomy lane 은 별도 명시 opt-in 단계에서만 활성화한다.
