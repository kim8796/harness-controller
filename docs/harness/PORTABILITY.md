...
현재 external controller 이식 UX는 `./harness install /path/to/product`, `./harness goal "제품 목표"`, `./harness watch`로 시작한다. `install`은 product repo를 등록하면서 controller-local runtime readiness를 확인/준비한다. `.venv`는 controller checkout 안의 runtime일 뿐 portability artifact가 아니며 product repo에 복사하지 않는다. `goal`은 단일 작업이 아니라 제품 완성 목표이고, `watch`가 목표를 planner/task로 나누어 구현, 검증, product commit, task branch push, PR publication receipt, 조건부 PR merge, local base sync를 반복한다. `./harness do "요청"`은 한 작업만 즉시 처리하는 보조 명령이다. `task review/queue`, `run`, `finish`, `target archive`, `target remove`는 같은 strict 게이트를 노출하는 고급/복구 표면이다.
...
새 프로젝트 이식의 기본 진입점은 repo-local 또는 starter-bundle-local `./harness` CLI 다. `./harness` 와 `./harness help` 는 한국어 beginner home 을 보여주고, `./harness --help` 는 고급 argparse 명령 참조다. Product repo 에 harness runtime/state 를 커밋하지 않는 운영은 controller 에서 `./harness install <repo>`, `./harness goal "제품 목표"`, `./harness watch` 를 우선 사용한다. 이 beginner path 는 내부적으로 controller-local runtime readiness, `target add`, `target set`/`target set-default`, goal sidecar store, deterministic planner/refill, task intake review/queue, `target run @default --implement-backlog-once`, target backlog transition/commit, task branch PR publication receipt, safe PR auto-merge/local sync 로 위임한다. 여러 product target 을 등록한 controller 에서는 `./harness fleet status` 가 target별 readiness/goal/backlog/watch/operator-wait와 `targets/_global/memory` 의 reusable learning 현황을 read-only 로 보여준다. `./harness watch --max-cycles 1 --no-telegram-drain` 은 실전 bounded smoke 이며, `./harness watch --status` 는 `targets/<id>/watch/latest.json` / `.md` 의 target-scoped heartbeat 와 다음 조치를 보여준다. `./harness install` 의 optional setup은 controller checkout의 tools와 `.venv`만 대상으로 하며 product repo, product secrets, product harness files를 만들지 않는다. macOS + Homebrew + TTY에서만 누락된 필수 도구 설치를 물어볼 수 있고, unsupported OS/Homebrew 없음/non-TTY에서는 자동 설치 없이 next action을 출력한다. `./harness do "요청"`은 같은 task intake gate를 쓰는 단일 작업 helper다. `./harness task/list/review/queue/fix-scope`, `./harness run`, `./harness finish` 는 복구/디버깅용 고급 명령으로 유지한다. 고급 검토는 `./harness target run <id|@alias|@default> --plan-once` 로 하고, 이 명령은 `targets/<id>/backlog` 만 읽고 product repo 를 바꾸지 않는다. 더 이상 관리하지 않을 product repo 는 `./harness target remove <id|@alias|@default>` 로 controller 등록만 `targets/_archived/` 아래로 옮긴다. 이 remove는 product repo 파일을 삭제하거나 수정하지 않고, active goal/queued backlog/operator-wait/run lock 을 먼저 확인한다. v1.8.5 부터 `./harness target run <id|@alias|@default> --execute-backlog-once` 는 같은 sidecar backlog 후보에 묶인 local `product-smoke-change.txt` diff 만 만들며 AI 구현 lane, backlog 완료 처리, commit, push 는 하지 않는다. v1.8.6 부터 `./harness target run <id|@alias|@default> --implement-backlog-once` 는 같은 후보를 AI implementer 에 넘겨 local product diff 만 만든다. Bare `./harness run` 은 lower-level one-shot autopilot wrapper이며, queued auto backlog 한 묶음을 처리한다. Continuous goal loop는 `./harness watch`가 맡는다. `finish`와 `target backlog push`는 legacy direct-push 복구 gate로 남아 있지만 beginner publication은 task branch PR receipt가 기본이다. `./harness verify --loop-ready` 는 필수 파일, git clean 상태, `.env` 추적 여부, bootstrap 문서/backlog, Telegram/relay readiness 를 secret 값 없이 점검한다. `./harness env check --provider vercel|upstash` 는 외부 env 준비를 present/missing/weak 상태로만 보여준다. `./harness self install` 은 선택형 global wrapper 를 만들지만 canonical entrypoint 는 계속 local `./harness` 다.

External controller release 는 source repo pre-push guard 와 분리한다. `v1.8.20` 부터 private controller repo 에서는 tag/release 전에 `./harness controller release-check --run-lint --run-pytest` 를 실행해 controller export source, `targets/` git-ignore, tracked forbidden state/secrets, focused lint/test 를 확인한다. `v1.8.23` 부터 root `START_HERE.md` 와 `docs/harness/START_HERE.md` 는 각각의 위치 기준으로 링크가 유효해야 하며, 이식 안내는 짧은 `START_HERE.md` 에서 시작하고 operator commands / task intake / Telegram / troubleshooting / starter scaffold reference 로 분리된 문서에 연결한다. Goal-driven external controller 는 `install -> goal -> watch` 를 beginner path 로 삼고, `watch` 는 active goal refill, implementation, transition, commit, task branch PR publication receipt, safe auto-merge/local sync 를 연결한다. `./harness controller audit-size|cleanup` 은 controller-owned smoke/temp sidecar 만 정리한다. 이 release-check 명령은 read-only 이며 GitHub release, product repo, target sidecar, env 값을 바꾸지 않는다.

External controller preview 는 product repo 에 harness runtime 파일을 커밋하지 않기 위한 새 경계다. `./harness controller doctor` 는 controller checkout 을 점검하고, `./harness target add <id> --repo <path>` 는 product git repo 를 controller 의 ignored `targets/<id>/` sidecar 에 등록한다. 이 preview 는 registry/projection/preflight 와 v1.8.0 RootContext state plumbing smoke 를 제공하며, 기존 backlog/GOALS/run parser 를 복제하지 않는다. v1.8.1 부터 product diff smoke 는 `./harness target run <id> --execute-once` 로만 활성화하고, `product-smoke-change.txt` local diff 하나와 sidecar evidence/rollback guidance 를 남긴다. v1.8.2 부터 `--execute-once --commit` 은 같은 deterministic smoke 파일만 local commit 으로 닫을 수 있지만 push 는 하지 않는다. v1.8.3 부터 advanced smoke `--execute-once --commit --push` 는 registered branch upstream 이 local before HEAD 와 일치할 때만 exact refspec 으로 push 하며, product repo push automation 이 실행될 수 있고 자동 remote rollback 은 하지 않는다. v1.8.4 부터 `--plan-once` 는 `targets/<id>/backlog` 의 queued auto backlog 후보를 같은 canonical parser 로 고른 뒤 report 에만 남긴다. v1.8.5 부터 `--execute-backlog-once` 는 그 후보를 hidden RootContext path 에서 다시 검증한 뒤 uncommitted smoke diff 하나만 만들고, backlog state 는 바꾸지 않는다. v1.8.6 부터 `--implement-backlog-once` 는 같은 후보를 AI implementer 에 넘겨 local product diff 만 만들고 backlog completion / commit / push 는 하지 않는다. v1.8.13 부터 `--implement-backlog-once` 의 기본 Codex 호출은 unsupported `-m auto` 를 만들지 않고 managed latest/default 모델 + `xhigh` reasoning 으로 실행된다. v1.8.8 부터 `target backlog transition` 이 completed / blocked / manual-review 상태 변경을 맡고, completed 는 passing implementation evidence 와 현재 product diff 일치가 필요하다. v1.8.9 부터 `target backlog commit` 이 completed backlog 에 연결된 implementation diff 를 local product commit 으로 닫을 수 있다. v1.8.12 부터 `target backlog push` 가 matching commit receipt 와 remote base 를 확인한 뒤 product remote branch 를 갱신할 수 있다. Hooks/GPG signing 을 건너뛰는 smoke/commit-gate 설명은 explicit local commit gates 에만 적용되며 Telegram-triggered delivery 가 아니다.
...
starter baseline 은 계속 `START_HERE.md` 기준을 따른다. happy path 는 `./harness new|init`, `./harness complete-setup --apply`, `./harness verify --loop-ready`, 필요 시 `./harness env check --provider vercel|upstash` 또는 controller `./harness telegram setup --target-id <id> --repo-id <repo> --dry-run`, `./harness run --once` 순서다. 설치 후 갱신은 `./harness upgrade --source <starter-bundle>` preview 후 `--apply` 로만 수행한다. `minimal` / `telegram` profile metadata 는 `scripts/harness_profiles.py` 가 맡는다. raw Python wizard 명령은 advanced implementation path 로만 취급한다. `POLICY.md` 는 이 저장소에서 실증이 끝날 때까지 optional extension 으로만 취급한다.
