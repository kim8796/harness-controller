...
- `v1.8.26` 기준 external controller 의 beginner 기본 실행은 `./harness run` autopilot 이다. default target 의 현재 queued auto backlog 를 처리하고 queue 가 비면 종료하며, 성공한 항목은 기존 gate 로 implement, complete, commit, push preflight/apply 까지 묶어 닫는다. 단일 transaction 점검은 `./harness run --once`, 지속 감시는 `./harness run --watch` 를 사용한다. Telegram/Redis relay drain 은 controller-owned Upstash adapter 를 쓰며 manual smoke 는 `--target-id <target>` 를 명시한다. Telegram setup readiness 는 `./harness telegram setup --target-id <id> --repo-id <repo> --dry-run` 으로 확인하고, dry-run 은 env/provider/webhook/deploy side effect 를 막는다.
