...
- `v1.8.25` external controller beginner path makes bare `./harness run` a long-running autopilot for the default target. It repeatedly selects queued auto sidecar backlog and closes each successful transaction through the existing implementation, completion, product commit, and push gate. `./harness run --once` remains the single-transaction debug path. If push preflight is not safe, the loop stops after any safe local commit and prints the next command instead of moving to the next backlog. Product pollution, secret-like diffs, dirty target state, detached HEAD, branch mismatch, remote drift, validation failure, and manual-review stop the loop fail-closed. Telegram/Redis relay drain uses a controller-owned Upstash adapter and target-scoped manual smoke uses `--target-id <target>`.
