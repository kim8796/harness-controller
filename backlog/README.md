...
- `v1.8.12` external controller 는 implementation run 뒤에도 backlog 완료, product commit, product push 를 자동 수행하지 않는다. `target backlog transition` dry-run 후 `--apply` 에서만 sidecar backlog 상태를 바꾸고, `target backlog commit` dry-run 후 `--apply` 에서만 completed backlog 의 product diff 를 local commit 으로 닫는다. 원격 push 는 matching commit receipt 와 remote base 를 확인하는 `target backlog push` dry-run 후 `--apply` 에서만 수행한다. Controller export 는 v1.8+ release note 이력을 보존하고 generated coverage artifact 를 제외한다.
