...
- External controller mode 에서는 product repo 의 `backlog/` 를 기본으로 만들거나 커밋하지 않는다. Controller 의 ignored `targets/<id>/` sidecar 가 harness state root 이고, target sidecar 경로는 `StatePaths` 로만 투영한다. `target run --once` 는 v1.7.107 기준으로 verified target 에서 read-only/no-op smoke 까지만 성공하며 backlog selection 이나 product-changing lane 을 시작하지 않는다. 이 smoke 는 v1.7.104 target별 lock 을 사용하고, product git HEAD/status before-after 를 비교해 `targets/<id>/reports/target-run-latest.md` 를 남긴다. Dirty target, branch mismatch, detached HEAD, post-smoke status/HEAD drift, product repo harness marker 는 fail-closed blocker 다. Telegram/Redis relay 는 multi-target 준비 시 signed `target_id` 와 target-scoped Redis keys 를 사용하고, v1.7.105 부터 local drain 이 `targets/<id>/operator-inbox` 로만 owner instruction 을 materialize 한다. v1.7.108 부터 controller target alias/default 는 operator selector 일 뿐이며 backlog/state/root paths 는 canonical target id 기준으로만 유지한다. v1.7.108 controller bundle 은 private repo CI 가 실행할 Node 24-compatible workflow, hosted-runner-safe focused tests, generated controller conftest, target-aware bridge/relay tests, target run read-only smoke tests 를 포함하지만, starter bundle 은 workflow/test files 를 제외한다. 실제 product diff 실행은 별도 RootContext-aware execution phase 에서만 켠다.
