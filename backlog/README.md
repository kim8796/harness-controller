...
- `v1.8.26` adds `./harness telegram setup` as the secret-safe Telegram/Redis setup wizard. `--dry-run` blocks every file/provider/webhook/deploy side effect, Vercel env sync is non-destructive by default, and gateway operator identity uses `HARNESS_TELEGRAM_OPERATOR_USER_IDS`.
- `v1.8.26` external controller beginner path makes bare `./harness run` drain the default target's currently queued auto backlog and exit when the queue is empty. It closes each successful transaction through the existing implementation, completion, product commit, and push gate. `./harness run --once` remains the single-transaction debug path, and `./harness run --watch` is the explicit long-running polling mode. If push preflight is not safe, the loop stops after any safe local commit and prints the next command instead of moving to the next backlog. Product pollution, secret-like diffs, dirty target state, detached HEAD, branch mismatch, remote drift, validation failure, and manual-review stop the loop fail-closed. Telegram/Redis relay drain uses a controller-owned Upstash adapter and target-scoped manual smoke uses `--target-id <target>`. Telegram setup readiness starts with `./harness telegram setup --target-id <id> --repo-id <repo> --dry-run`, which redacts values and does not mutate env/provider/webhook/deploy state.
- `v1.8.24` controller retention keeps default implementation smoke disposable. `./harness smoke implementation` does not retain `targets/smoke-*` unless `--keep` is explicit. `./harness controller audit-size` reports target/run/report/outbox size and cleanup candidates. `./harness controller cleanup --dry-run|--apply` only removes controller-owned delete-safe smoke/temp sidecars and compactable evidence; product repo files, active targets, queued/active backlog, receipts, commit/push evidence, and latest dashboards are protected.
- `v1.8.23` external controller 는 bare `./harness` / `./harness help` 에서 한국어 beginner home 을 보여주고, root `START_HERE.md` 는 root-relative `docs/harness/...` 링크만 사용한다. Shorter `./harness install /path/to/product --id my-app --default` 형태와 TTY install prompt 를 지원한다. `./harness task list` 는 sidecar task packet 의 review/queue 상태와 packet-specific 다음 명령을 read-only 로 보여주고, stale review 는 `다시 검토 필요` 로 표시한다. `task review` 는 safe config alias 를 exact 후보 파일명으로 보정하고, `task fix-scope` 는 scope-only manual-review dead end 를 auto queue 로 복구할 수 있다. Implementation run 기본 Codex 호출에서 literal `auto` 모델을 넘기지 않고 managed latest/default 모델과 `xhigh` reasoning 을 사용한다. Implementation run 뒤에도 backlog 완료, product commit, product push 를 자동 수행하지 않는다. Beginner `./harness finish` 는 다음 단계를 보여주는 읽기 전용 요약이고, `finish --apply` 에서만 sidecar backlog 상태를 바꾼다. `finish --commit --message ...` 와 `finish --push` 는 dry-run 이며 실제 local commit / remote push 는 각각 `--apply` 를 추가했을 때만 기존 gate 로 수행한다. `START_HERE.md` 는 짧은 entrypoint 로 유지하고 detailed task/operator/Telegram/troubleshooting/scaffold 내용은 dedicated docs 로 연결한다. Controller export 는 v1.8+ release note 이력을 보존하고 generated coverage artifact 를 제외한다. Private controller release 전에는 `./harness controller release-check` 로 controller bundle readiness, forbidden tracked path, ignored sidecar state, optional focused lint/pytest 를 확인한다.
- Portable starter bootstrap 은 `START_HERE.md` 의 quick start 를 기준으로 새 project 생성, independent bundle, existing repo install 중 하나를 먼저 고른다. 기본 경로는 `./harness new <dir>` 또는 `./harness init <repo>` 이고, 다른 곳으로 옮길 starter pack 은 `./harness export <dir>` 로 만든다. Product repo 에 harness runtime/state 를 커밋하지 않는 controller 운영은 `./harness controller export <dir>` 로 private controller bundle 을 만든 뒤 `./harness` / `./harness help` 의 beginner home 과 `install -> task -> task list -> task review <packet-id> -> task queue <packet-id> --auto -> run -> finish` 를 사용한다. 이미 manual-review 로 들어간 scope-only task 는 `task fix-scope <packet-id> --apply` 로 복구할 수 있다. 이미 설치한 starter 는 `./harness upgrade --source <bundle>` preview 후 `--apply` 로 starter-safe harness files 만 갱신한다. 이후 `./harness complete-setup --apply` 로 starter draft 를 적용하고 `./harness verify --loop-ready` 로 설치 상태, bootstrap backlog, Telegram/relay readiness 를 secret 값 없이 확인한다. Vercel/Upstash 준비 상태는 `./harness env check --provider vercel|upstash` 로 present/missing/weak 만 확인한다. 독립 bundle 로 설치한 경우도 같은 backlog 규칙을 따른다. v1.8.23 beginner controller task intake 는 `./harness task` guided interview -> `./harness task list` 상태 확인 -> `./harness task review <packet-id>` deterministic review -> optional `./harness task review <packet-id> --ai` advisory artifacts -> `./harness task queue <packet-id> --auto` 순서로 queued sidecar backlog 를 만들며, AI review 는 queue gate 가 아니다. Wizard 가 만든 `Autonomy-Execute: auto` 는 validation command, file scope, credential/destructive-risk checks 를 통과한 경우에만 허용하고, 불명확하면 `manual-review` 로 둔다.
...
- External controller mode 에서는 product repo 의 `backlog/` 를 기본으로 만들거나 커밋하지 않는다. Controller 의 ignored `targets/<id>/` sidecar 가 harness state root 이고, target sidecar 경로는 `StatePaths` 로만 투영한다. `target run --once` 는 v1.8.0 기준으로 verified target 에서 RootContext-aware state plumbing 을 포함한 read-only/no-op smoke 까지만 성공하며 backlog selection 이나 product-changing lane 을 시작하지 않는다. 이 smoke 는 target별 `target-run.lock` token 을 사용하고, product git HEAD/status before-after 를 비교해 `targets/<id>/reports/target-run-latest.md` 와 `targets/<id>/reports/harness-autonomy`, `targets/<id>/runs/harness`, `targets/<id>/operator-outbox` evidence 를 남긴다. Dirty target, branch mismatch, detached HEAD, post-smoke status/HEAD drift, product repo harness marker 는 fail-closed blocker 다. Telegram/Redis relay 는 multi-target 준비 시 signed `target_id` 와 target-scoped Redis keys 를 사용하고, v1.7.105 부터 local drain 이 `targets/<id>/operator-inbox` 로만 owner instruction 을 materialize 한다. v1.7.108 부터 controller target alias/default 는 operator selector 일 뿐이며 backlog/state/root paths 는 canonical target id 기준으로만 유지한다. v1.8.1 기준 `target run --execute-once` 는 explicit local product diff smoke 로 `product-smoke-change.txt` 하나만 만들고 commit/push 는 하지 않는다. v1.8.2 기준 `target run --execute-once --commit` 은 같은 파일만 local smoke commit 으로 닫고 push 는 하지 않으며, rollback 은 `target-run-latest.md` 에 기록된 before HEAD 를 쓰되 HEAD가 아직 smoke commit일 때만 실행한다. v1.8.3 기준 `target run --execute-once --commit --push` 는 advanced smoke 로 registered branch remote 를 갱신할 수 있고 product repo push automation 이 실행될 수 있으므로 quick path 에 두지 않는다. v1.8.4 기준 `target run --plan-once` 는 `targets/<id>/backlog/queued` 의 `Status: queued` + `Autonomy-Execute: auto` 후보만 고르고 product repo backlog 나 product diff 를 만들지 않는다. v1.8.5 기준 `target run --execute-backlog-once` 는 같은 후보를 hidden RootContext path 에서 다시 검증한 뒤 local smoke diff 하나만 만들고 backlog 파일은 변경하지 않는다. v1.8.6 기준 `target run --implement-backlog-once` 는 같은 후보를 AI implementer 에 넘겨 local product diff 만 만들고 backlog 파일, commit, push 는 변경하지 않는다. v1.8.13 기준 이 구현 gate 의 기본 Codex 호출은 unsupported `auto` 모델을 넘기지 않고 managed latest/default 모델과 `xhigh` reasoning 을 사용한다. v1.8.8 기준 `target backlog transition` 은 completed / blocked / manual-review 전환을 담당하며, completed 는 passing implementation evidence 와 현재 product diff 일치를 요구한다. v1.8.9 기준 `target backlog commit` 은 completed backlog, matching `Completed-Run`, 현재 product diff path/fingerprint 를 요구한 뒤 evidence-listed product path 만 local commit 으로 닫는다. v1.8.12 기준 `target backlog push` 는 matching product commit receipt 와 remote base 를 요구한 뒤 registered upstream 을 갱신할 수 있다. 자동 remote rollback 은 없고 revert/recovery 는 operator 가 repo 정책에 맞춰 판단한다. v1.8.13 controller bundle 은 private repo CI 가 실행할 Node 24-compatible workflow, hosted-runner-safe focused tests, generated controller conftest, target-aware bridge/relay tests, state plumbing smoke tests, sidecar backlog plan tests, backlog-bound diff smoke tests, managed-latest/xhigh backlog implementation tests, backlog transition tests, backlog commit/push tests, product diff smoke tests, local smoke commit/push tests, v1.8+ release note history 를 포함하고 generated coverage artifact 를 제외하지만, starter bundle 은 workflow/test files 를 제외한다. 일반 product delivery push 는 explicit gate 에서만 켠다.
