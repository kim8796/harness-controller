...
def _add_maintainability_docs(repo: Path) -> None:
    (repo / "docs").mkdir(exist_ok=True)
    (repo / "README.md").write_text("# Product\n\nOwner and operating summary.\n", encoding="utf-8")
    for name in ("ARCHITECTURE.md", "OPERATIONS.md", "CODEMAP.md", "TESTING.md"):
        (repo / "docs" / name).write_text(f"# {name}\n\nMaintained product handoff note.\n", encoding="utf-8")
    subprocess.run(["git", "add", "."], cwd=repo, check=True)
    subprocess.run(["git", "commit", "-m", "maintainability docs"], cwd=repo, check=True, stdout=subprocess.PIPE)
...
def test_command_watch_max_cycles_with_pending_gates_leaves_actionable_state(tmp_path, capsys) -> None:
    ...
    _add_maintainability_docs(product)
...
def test_command_watch_routes_external_gate_reasons_without_idle_no_progress(tmp_path, capsys) -> None:
    module = _load_module()
    module.ERROR_CLASS = RuntimeError
    controller = tmp_path / "controller"
    product = tmp_path / "product"
    ...
    module.harness_goal.refresh_progress(state_root=state_root, goal=goal)
    ...
    runtime = SimpleNamespace(
        ...
        refill_goal_if_idle=lambda _record: {
            ...
            "reason_by_gate": {gate_id: reason for gate_id in pending_gate_ids},
        },
        pending_backlog_product_pushes=lambda **_kwargs: [],
        auto_merge_pending_publications=None,
        github_credentials_ready=lambda **_kwargs: True,
    ...
    status = json.loads((state_root / "watch" / "latest.json").read_text(encoding="utf-8"))
    assert status["phase"] == "max-cycles-action-required"
    assert status["status"] in {"blocked", "operator-wait"}
    assert status["next_action_kind"] != "publication-actionable"
    assert "production_e2e_smoke" in status["gate_route"]["by_kind"]["setup-actionable"]
    assert "production_e2e_smoke" not in status["gate_route"]["by_kind"].get("publication-actionable", [])
    assert status["phase"] != "max-cycles-idle-no-progress"
