...
`v1.8.8` 기준 external controller 는 `--implement-backlog-once` 뒤에도 backlog 를 자동 완료하지 않는다. `./harness target backlog transition <target> --status completed|blocked|manual-review` 로 dry-run 한 뒤 `--apply` 에서만 sidecar backlog 상태를 바꾼다.
